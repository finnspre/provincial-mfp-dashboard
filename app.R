...
# Builds the Growth Accounting tab's Interval choices -- "1 year (annual)"
# through "<max_years> years" -- for a given maximum, no fixed cap (there
# used to be one, at 10; removed per explicit request). `max_years` is
# always the *current* date range's own span (see growth_interval_max()),
# so the choices offered are exactly the Intervals that range can actually
...
# (period width, step size); selectizeInput() still reports a picked one
# back as a string like every HTML <select> (see growth_interval()'s own
# as.integer() round-trip), same convention the Trends/Compare tabs' own
# numeric Base year <select> already relies on.
growth_interval_choices <- function(max_years) {
  if (max_years < 1) return(setNames(integer(0), character(0)))
  labels <- if (max_years == 1) "1 year (annual)" else c("1 year (annual)", paste0(2:max_years, " years"))
...
# to as many years wide as the date range itself spans (no fixed cap --
# picking the widest available Interval always gives exactly 1 period, the
# fewest possible) -- so how many periods a given date range produces
...
# GROWTH_ACCOUNTING_VARS), so the annual case ("1 year (annual)", explicitly
# picked) crosses this threshold at the default (full) date range, even
# though the *default* Interval itself (blank -- see growth_interval()'s own
# comment) never does, since it always collapses to a single period.
  ...
  card(
    ...
    layout_sidebar(
      sidebar = sidebar(
        ...
        # How many years each bar covers -- blank by default (per explicit
        # request), meaning "as long as the selected date range itself is",
        # i.e. one single bar for the whole thing (see growth_interval()'s
        # own comment for how that default is computed). No fixed cap on
        # the choices offered (there used to be one, at 10 years; removed
        # per explicit request) -- growth_interval_choices() below builds
        # "1 year (annual)" through "<the date range's own span> years"
        # fresh for whatever init_df's full range happens to be, and
        # growth_tab_server()'s own sync observer regenerates that same
        # list from scratch every time input$year_range changes, so the
        # choices on screen are always exactly the Intervals the *current*
        # range can support -- never a stale list built for a different
        # range. This is a typable selectizeInput (unlike every other plain
        # single-value dropdown in this app, e.g. the Trends tab's Base
        # year) besides, so a reader can also just type a number rather
        # than scrolling a potentially long list; create=TRUE is what lets
        # a typed value outside the current `choices` be accepted at all
        # (a narrower range hasn't caught up with a re-render yet), and
        # createFilter restricts what typing can even attempt to create to
        # a bare positive integer (so e.g. letters never become a pending
        # "create" option) -- growth_interval() is still what actually
        # clamps an accepted-but-too-large number down to the widest
        # interval the current date range can support, both here and
        # server-side (see that reactive's own comment). Periods are
        ...
        selectizeInput(
          ...
          choices = c(setNames("", ""), growth_interval_choices(max(init_df$Year) - min(init_df$Year))),
          selected = "",
          options = list(
            create = TRUE, createFilter = "^[1-9][0-9]*$",
...
growth_tab_server <- function(id, raw_data) {
  moduleServer(id, function(input, output, session) {
    ...
    # input$interval as a clean, defensive integer >= 1 -- selectizeInput()
    ...
    # it's coerced once here rather than 3 times over. Blank ("", the
    # selectize box's own default -- see growth_tab_ui()'s own comment) and
    # anything unexpected (NULL before the input first exists, or a
    # somehow-invalid value) both fall back to growth_interval_max() -- one
    # single bar covering the whole date range, per explicit request, and
    # (same as the old fallback of 1 before this) an Interval that can
    # never itself produce a truncated period, since it exactly *is* the
    # period. A typed-or-picked value bigger than the range can support is
    # clamped down to that same max -- "automatically redirect to the
    # largest possible interval", per explicit request; the observer just
    # below keeps the box's own displayed value in sync with that clamp
    # rather than silently showing a too-large number the chart doesn't
    # actually use.
    ...
    # Regenerates the Interval box's own clickable choices from scratch
    # every time the date range changes, so the list on screen always spans
    # exactly "1 year (annual)" through however many years the *current*
    # range covers (see growth_interval_choices()'s own comment) -- no
    # fixed cap on offer, per explicit request (there used to be one, at 10
    # years). Also snaps the box's own selected value down if it's now
    # wider than the new, narrower range can support (e.g. a 20-year
    # Interval chosen over a wide range, then the range itself narrowed
    # without first clearing the Interval) -- the same clamp
    # growth_interval() applies for the chart itself, kept in sync here so
    # the box never shows a number the chart isn't actually using. Blank is
    # left untouched either way (already means "the whole range", nothing
    # to clamp or regenerate away from).
    observe({
      ...
      updateSelectizeInput(
        session, "interval",
        choices = c(setNames("", ""), growth_interval_choices(current_max)), selected = selected
      )
    }) |> bindEvent(input$year_range, ignoreInit = TRUE)

    # Snaps a typed-or-picked Interval value back down to the current date
    # range's own width the moment it's entered, without waiting for a
    # date-range change to trigger the choices-regenerating observer above
    # -- "automatically redirect to the largest possible interval", per
    # explicit request. Only fires when the box is showing something that
    # needs correcting (strictly greater than the max) -- never touches a
    # blank box (already means "the whole range") or an in-range number, so
    # this doesn't fight a mid-typing selectize entry that's still within
    # bounds.
    observe({
      iv <- suppressWarnings(as.integer(input$interval))
      current_max <- growth_interval_max()
      if (length(iv) > 0 && !is.na(iv) && iv > current_max) {
        updateSelectizeInput(session, "interval", selected = as.character(current_max))
      }
