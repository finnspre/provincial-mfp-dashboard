...
# Builds the Growth Accounting tab's Interval choices -- "1 year" through
# "<max_years> years" -- for a given maximum, no fixed cap (there used to
# be one, at 10; removed per explicit request). `max_years` is always the
# *current* date range's own span (see growth_interval_max()), so the
# choices offered are exactly the Intervals that range can actually
# support, not an arbitrary fixed list -- regenerated by growth_tab_ui()
# once for the initial range and by growth_tab_server()'s own sync observer
# every time input$year_range changes thereafter. max_years < 1 (a
# degenerate single-year range) has no valid Interval at all -- an empty
# choice list, rather than a "0 years" entry that could never actually be
# used. Values are plain integers (not e.g. "5y" strings) since
# growth_tab_server() does arithmetic directly on the selected interval
# (period width, step size); treeSelectInput() still reports a picked one
# back as a plain character(1) (see growth_interval()'s own as.integer()
# round-trip), same contract every other picker in this app built on that
# widget already relies on.
growth_interval_choices <- function(max_years) {
  if (max_years < 1) return(setNames(integer(0), character(0)))
  labels <- if (max_years == 1) "1 year" else c("1 year", paste0(2:max_years, " years"))
  setNames(seq_len(max_years), labels)
}
...
# filtered_data()) -- 1-year-wide periods at Interval "1 year", up to as
# many years wide as the date range itself spans (no fixed cap -- picking
# the widest available Interval always gives exactly 1 period, the fewest
# possible) -- so how many periods a given date range produces varies with
# that pick; table 36-10-0211-01 spans up to 26 years of usable growth data
# per industry (1997-2023, minus the first year -- see
# GROWTH_ACCOUNTING_VARS), so the annual case ("1 year", explicitly picked)
# crosses this threshold at the default (full) date range, even though the
# *default* Interval itself ("Full date range" -- see growth_interval()'s
# own comment) never does, since it always collapses to a single period.
# PX_PER_PERIOD budgets enough width per period for both bars (the labour
# productivity growth bar + the stacked "other factors" bar) plus the tight
# gap between them and a share of the wider gap to the next period -- see
# GROWTH_BAR_OFFSET/GROWTH_BAR_WIDTH below.
...
# node is a root, `children = list()` -- used for Variable/Geography (value
# and label the same string, `choices` unnamed) and the Growth Accounting
# tab's own Interval picker (value != label -- e.g. "" displays as "Full
# date range", "5" as "5 years" -- `choices` a named vector, name = label),
# where industry_tree_nodes()'s parent-walk doesn't apply either way.
# `choices` is expected already in display order (e.g. series_choices()'s
# VARIABLE_ORDER, or growth_interval_choices()'s own ascending build);
# unlike industry_tree_nodes(), this doesn't re-sort. names(choices) is
# NULL (not a same-length vector of "") for the plain unnamed case every
# existing caller before Interval used, which is what lets that case keep
# falling through to the old value-is-its-own-label behaviour unchanged.
flat_tree_nodes <- function(choices) {
  labels <- if (is.null(names(choices))) as.character(choices) else names(choices)
  Map(function(value, label) list(value = value, label = label, children = list()), unname(choices), labels, USE.NAMES = FALSE)
}
  ...
  card(
    ...
    layout_sidebar(
      sidebar = sidebar(
        ...
        # How many years each bar covers -- "Full date range" (value "", the
        # default) means one single bar for the whole thing (see
        # growth_interval()'s own comment for how that's computed), and is
        # always the first, permanent entry in the list -- present no
        # matter what growth_interval_choices() below generates for the
        # rest, not just an initial default that could later scroll out of
        # the list (per explicit request). No fixed cap on the numbered
        # choices after it (there used to be one, at 10 years; removed per
        # earlier explicit request) -- growth_interval_choices() builds "1
        # year" through "<the date range's own span> years" fresh for
        # whatever init_df's full range happens to be, and
        # growth_tab_server()'s own sync observer regenerates that same
        # list (with "Full date range" still first) every time
        # input$year_range changes, so the choices on screen are always
        # exactly the Intervals the *current* range can support. The same
        # treeSelectInput widget every other picker in this app's sidebars
        # uses (see the matching comment on this tab's own Industry/
        # Geography pickers just above) -- searchable-to-filter like they
        # are (per explicit request: match their look/feel, not the plain
        # native <select> this control used in between), via
        # flat_tree_nodes() over a *named* vector rather than the plain
        # unnamed one every other flat_tree_nodes() call site here passes
        # (value != label -- "" reads as "Full date range", "5" as "5
        # years" -- see flat_tree_nodes()'s own comment for how it tells
        # the 2 cases apart). Still not a *creatable* box, unlike a bare
        # selectizeInput with create=TRUE (tried, then reverted 2 requests
        # ago): the list already enumerates every legal Interval for the
        # current range, so there's nothing valid left to type that isn't
        # already a row to search down to and click. Periods are counted
        # backward from the date range's own *end* year (see
        # filtered_data()), not forward from its start, so choosing e.g.
        # "5 years" over 2008-2020 gives 2015-2020/2010-2015/2008-2010, the
        ...
        treeSelectInput(
          ...
          tree_data = flat_tree_nodes(c("Full date range" = "", growth_interval_choices(max(init_df$Year) - min(init_df$Year)))),
          selected = "", placeholder = "Search intervals..."
        ),
...
growth_tab_server <- function(id, raw_data) {
  moduleServer(id, function(input, output, session) {
    ...
    # The current date range's own width in years -- the "largest possible
    # interval" growth_interval() below falls back/clamps to, per explicit
    # request. req() (not a default c(...) value) because every consumer of
    # growth_interval() already assumes input$year_range exists by the time
    # it runs (see e.g. filtered_data()'s own req(input$year_range)).
    growth_interval_max <- reactive({
      req(input$year_range)
      max(input$year_range[2] - input$year_range[1], 0L)
    })

    # input$interval as a clean, defensive integer >= 1 -- treeSelectInput()
    # always reports its value back as a plain character(1) (see
    # growth_interval_choices()'/flat_tree_nodes()'s own comments), and
    # this is read from 3 different places below (the
    # period math itself, the chart's title/axis, and the CSV filename), so
    # it's coerced once here rather than 3 times over. Blank ("", "Full
    # date range" -- see growth_tab_ui()'s own comment) and anything
    # unexpected (NULL before the input first exists, or a somehow-invalid
    # value) both fall back to growth_interval_max() -- one single bar
    # covering the whole date range, per explicit request, and (same as
    # the old fallback of 1 before this) an Interval that can never itself
    # produce a truncated period, since it exactly *is* the period. A
    # picked value bigger than the range can support is clamped down to
    # that same max as a defensive last resort -- in practice this can't
    # come from a click any more (the observer just below regenerates the
    # list itself, so every clickable option is already <= the current
    # max), but a stale in-flight input during that same regeneration is
    # cheap insurance against.
    growth_interval <- reactive({
      iv <- suppressWarnings(as.integer(input$interval))
      if (length(iv) == 0 || is.na(iv) || iv < 1) growth_interval_max() else min(iv, growth_interval_max())
    })

    # Regenerates the Interval box's own clickable/searchable choices from
    # scratch every time the date range changes, so the list on screen
    # always spans "Full date range" (always first, always present -- per
    # explicit request) plus exactly "1 year" through however many years
    # the *current* range covers (see growth_interval_choices()'s own
    # comment) -- no fixed cap on offer, per earlier explicit request
    # (there used to be one, at 10 years). Also snaps the box's own
    # selected value down if it's now wider than the new, narrower range
    # can support (e.g. a 20-year Interval chosen over a wide range, then
    # the range itself narrowed without first picking "Full date range")
    # -- the same clamp growth_interval() applies for the chart itself,
    # kept in sync here so the box never shows a number the chart isn't
    # actually using. "Full date range" itself is left untouched either
    # way (already means "the whole range", nothing to clamp or
    # regenerate away from). updateTreeSelectInput(), not
    # updateSelectInput() -- this picker is a treeSelectInput (see
    # growth_tab_ui()'s own comment), same update counterpart every other
    # tree-select sync observer in this app uses.
    observe({
      current_max <- growth_interval_max()
      iv <- suppressWarnings(as.integer(input$interval))
      selected <- if (length(iv) > 0 && !is.na(iv) && iv > current_max) as.character(current_max) else input$interval
      updateTreeSelectInput(
        session, "interval",
        tree_data = flat_tree_nodes(c("Full date range" = "", growth_interval_choices(current_max))),
        selected = selected
      )
